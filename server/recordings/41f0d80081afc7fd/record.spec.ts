...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Enter Email Address' }).fill('qadence@test');
  // Screenshot step-3.png
  await page.getByRole('textbox', { name: 'Enter Password' }).click();
  // Screenshot step-4.png
  await page.getByRole('textbox', { name: 'Enter Password' }).fill('1q2w3e4rr565');
  // Screenshot step-5.png
  await page.getByRole('button', { name: 'Login' }).click();
  // Screenshot step-6.png
  await page.getByRole('textbox', { name: 'Enter Email Address' }).fill('qadence@test.com');
